...
void decode_and_exec(chip8_t *chip8) {
	...
	case 0xE0: {
		...
		if (subflag == 0x9E) {
			...
			if (chip8->key == chip8->registers[vx]) {
				chip8->PC += 2;
			}
		}	
	...
	case 0xF0: {
		...
		case 0x1E:
			#ifdef DEBUG
			printf("Add %d to I = %d\n", chip8->registers[vx], chip8->I + chip8->registers[vx]);
			#endif
		...
		case 0x29:
			...
			#endif
			chip8->I = chip8->registers[vx] & 0x0F;
			break;
		case 0x33:
			#ifdef DEBUG
			printf("set BCD OP\n");
			...
			chip8->memory[chip8->I + 2] = (chip8->registers[vx] % 100) % 10;
			break;
		case 0x55: {
			size_t i = 0;
			while (i < VF) {
		...
		default:
			fprintf(stderr, "ERROR: ILLEGAL OPCODE in 0xF0");
			chip8->running = 0;
			return;
		}
		break;
